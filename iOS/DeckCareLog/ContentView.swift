...
struct EntryFormView: View {
    ...
    init(mode: EntryFormMode) {
        ...
        if case .edit(let entry) = mode {
            ...
            _maintenanceType = State(initialValue: entry.maintenanceType)
    ...
    var body: some View {
        NavigationStack {
            Form {
                DatePicker("Date Performed", selection: $date, displayedComponents: .date)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(maintenanceType.trimmingCharacters(in: .whitespaces).isEmpty)
                }
            }
        }
    }
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = EntryEntry(date: date, maintenanceType: maintenanceType, notes: notes)
            store.add(entry)
